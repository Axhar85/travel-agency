...
export function AdminPromoCardsDashboard() {
  const t = useTranslations("Admin");
  const router = useRouter();

  const [cards, setCards] = useState<PromoCardData[] | null>(null);
  ...
  async function load() {
    try {
      const list = await getAllPromoCards();
      setCards(list);
    } catch (error) {
  ...
  async function handleUpload(event: React.FormEvent) {
    ...
    setUploadError(null);
    try {
      await createPromoCard(imageFile, { titleEs, titleEn, subtitleEs, subtitleEn, linkUrl });
      setImageFile(null);
      setTitleEs("");
      setTitleEn("");
      setSubtitleEs("");
  ...
  async function handleToggleActive(card: PromoCardData) {
    await updatePromoCard(card.id, { isActive: !card.isActive });
    await load();
  }
  ...
  async function handleMove(index: number, direction: -1 | 1) {
    ...
    setCards(reordered);
    await reorderPromoCards(reordered.map((c) => c.id));
  }
  ...
  return (
    <div className="flex w-full max-w-2xl flex-col gap-8">
      <h1 className="text-xl font-semibold text-black">{t("promoCardsTitle")}</h1>
      ...
      <div className="flex flex-col gap-3">
        {cards.length === 0 && <p className="text-sm text-zinc-600">{t("noPromoCards")}</p>}
